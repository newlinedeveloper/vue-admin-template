<template>
  <a-layout-header :class="[theme, 'global-header']">
    <div :class="['global-header-wide', layout]">
      ...
      >
        <img width="32" :src="systemSmallLogo" />
  ...
  computed: {
    ...
    theme() {
      return this.layout === "side"
        ? "light"
        : this.$store.state.AppSetting.theme;
    },
    systemName() {
    ...
    systemSmallLogo() {
      if (
        this.$store.state.AppSetting.systemSmallLogo &&
        String(this.$store.state.AppSetting.systemSmallLogo).length > 0
      ) {
